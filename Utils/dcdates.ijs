...
Global variables called Linux0DateTime and J0Date are also available for 
performing similar translations to and from Linux-style dates (which are 
usually stored in seconds) and J-style day numbers (results of the todayno 
verb in the dates.ijs script). Other constants may be computed for any other 
special needs.
...
NB. =========================================================
NB. Verbs for working with dates and daynumbers
...
CookieExpires=: 3 : 0
  ...
  if. ' '={.0$ts do.                       NB. *****Reverse conversion from text
    ...
    if. 3<:#ts do.                         NB. using 6!:0 '' format
      ...
      if. date>:10000000 do.               NB. YYYYMMDD[.HHMMSS] form
        date=. 1 dJulian date
        time=. 60#.(3#100)#:<.0.5+time*1000000
      else.                                NB. Julian Day form
        time=. <.0.5+86400*time
      end.
    end.
...
dSpell=: 3 : 0
  ...
  NB. force unescaped occurences of D, M or Y to lowercase  
  'unesc pic'=. '\' escaped pic
  pic=. (tolower idx{pic)  (idx=. I. unesc *. pic e. 'DMY')} pic NB. force to l/c
  pic=. (1 , 2 ~:/\unesc *. pic e. 'dmy') <;.1 pic               NB. Cut into tokens
  var=. pic e. codes                                             NB. mark sections as vars
  
  s=. *ymd=. ,1 dGregorian <.,y
  ymd=. |: 0 100 100 #:|ymd
  ymd=. (s*0{ymd) (0)}ymd                  NB. handle negative years (BC)

  t=. 2{ymd                                NB. Days
  values=. ('';'r<0>2.0') fmt"0 1 t
  t=. (7|1+<.,y){WKDAYS                    NB. Day names
  values=. values,(3&{.&.> ,: ]) t
  t=. 1{ymd                                NB. Months
  values=. values, ('';'r<0>2.0') fmt"0 1 t
  t=. (0>.t){MONTHS                        NB. Month names
  values=. values, (3&{.&.> ,: ]) t
  s=. 0>:t=. 0{ymd                         NB. Years
  t=. |t-s                                 NB. -ve yrs are BC
  s=. s{('';' BC')
  t=. (fmt t), &.> s
  values=. values, (2&}.&.> ,: ]) t
...
NB.*tSpell v Format a time (in seconds) in a given format.
NB. y is: time in seconds since start of the day 
...
tSpell=: 3 : 0
  ...
  NB. force unescaped occurences of D, M or Y to lowercase  
  'unesc pic'=. '\' escaped pic
  pic=. (tolower idx{pic)  (idx=. I. unesc *. pic e. 'DHMSCP')} pic NB. force to l/c
...
CookieExpires_z_ =: CookieExpires_rgsjulian_
dGregorian_z_ =: dGregorian_rgsjulian_
dJulian_z_ =: dJulian_rgsjulian_
dSpell_z_ =: dSpell_rgsjulian_
tLocal_z_ =: tLocal_rgsjulian_
tSpell_z_ =: tSpell_rgsjulian_
tUTC_z_ =: tUTC_rgsjulian_
...
Note 'Examples'
 ...
 1 dJulian 3# 20090907.133031267
 dGregorian dJulian now
 1 dGregorian dJulian now
 assert (dJulian now)(0.01 > |@:-)  1 dJulian 1&dGregorian dJulian now
 dSpell dJulian now
 'DDDD, D MMMM YYYY' dSpell dJulian now
 'DDD, MMM DD, YYYY' dSpell dJulian now
 'DD/MM/YYYY' dSpell dJulian now
 'D/M/YY' dSpell dJulian now
 tSpell 0 60 60#. _3{. now
 'HH:MM:SS' tSpell 0 60 60#. _3{. now
)
Note 'Examples Output'
   ]now=: 6!:0 ''
2009 9 7 13 30 31.267
   dJulian now
2455082.56286
   dJulian 3#,:now
2455082.56286 2455082.56286 2455082.56286
   1 dJulian 20090907.133031267
2455082.56286
   1 dJulian 3#20090907.133031267
2455082.56286 2455082.56286 2455082.56286
